...
def parse_script(script: str) -> list[Segment]:
    ...
    while i < len(lines):
        ...
        if bg_music_match:
            ...
            dur = int(bg_music_match.group(2))
...
class AudiobookProducer:
    ...
    Orchestrates Kokoro (narrator), Qwen3-TTS (characters), and MusicGen (music/SFX)
    into a single assembled audiobook MP3.
    """
    
    # Standard sample rate for the final mix (Kokoro outputs 24kHz)
    OUTPUT_SR = 24000
    
    def __init__(self, bot):
        self.bot = bot
        # Voice cache: character_name -> path to reference audio clip
        # Ensures consistent voices throughout the audiobook
    ...
    async def produce(self, script: str, output_path: str, title: str = "Audiobook") -> str:
        ...
        logger.info("Assembling final audiobook...")
