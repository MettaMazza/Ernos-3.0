...
class LocalMediaGenerator:
    ...
    def generate_music(self, prompt: str, output_path: str, duration: int = 10) -> str:
        ...
        Args:
            ...
            duration: Length in seconds (default 10, supports >30 via chaining)
        ...
        TOKENS_PER_SEC = 50
        MAX_SINGLE_PASS = 30  # seconds — quality degrades beyond this
        
        sample_rate = model.config.audio_encoder.sampling_rate
        
        if duration <= MAX_SINGLE_PASS:
            # Single pass generation
            logger.info(f"MusicGen single pass: {duration}s — {prompt[:80]}...")
            ...
            # Segment chaining for longer compositions
            num_segments = -(-duration // MAX_SINGLE_PASS)  # ceil division
            segment_duration = min(MAX_SINGLE_PASS, duration)
            logger.info(f"MusicGen chaining: {num_segments} segments × {segment_duration}s — {prompt[:80]}...")
            
            segments = []
            for i in range(num_segments):
                # Vary prompt slightly for natural progression
                seg_prompt = prompt
                if i > 0:
                    seg_prompt = f"{prompt}, continuation, seamless transition"
                
                inputs = processor(text=[seg_prompt], padding=True, return_tensors="pt").to(self.device)
                
                with torch.no_grad():
                    audio = model.generate(**inputs, max_new_tokens=segment_duration * TOKENS_PER_SEC)
                
                segments.append(audio[0, 0].cpu().float().numpy())
                logger.info(f"MusicGen segment {i+1}/{num_segments} complete.")
            
            audio_data = chain_audio_segments(segments, sample_rate, crossfade_sec=2.0)
