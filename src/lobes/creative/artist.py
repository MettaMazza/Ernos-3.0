...
class VisualCortexAbility(BaseAbility):
    ...
    async def _produce_audiobook_background(self, script: str, user_id: int, channel_id: int,
        ...
        """Background task for audiobook production."""
        try:
            logger.info(f"Starting background audiobook production: '{title}' for {user_id}")
            
            from src.lobes.creative.audiobook_producer import AudiobookProducer
            producer = AudiobookProducer(self.bot)
            final_path = await producer.produce(script, output_path, title=title)
            
            if final_path and os.path.exists(final_path):
                # Log Provenance
                from src.security.provenance import ProvenanceManager
                ProvenanceManager.log_artifact(final_path, "audiobook", {
                    "title": title,
                    "user_id": user_id,
                    "scope": request_scope,
                    "is_autonomy": is_autonomy,
                ...
                if channel:
                    ...
                    if size_mb <= 25:  # Discord file size limit
                        file = discord.File(final_path)
                        await channel.send(
                            f"📖🎧 **Audiobook Complete: {title}**\n"
                        ...
                        await channel.send(
                            f"📖🎧 **Audiobook Complete: {title}**\n"
                            f"*File is {size_mb:.1f} MB (too large for Discord). "
                            f"Saved to: `{final_path}`*"
                        )
                    logger.info(f"Audiobook delivered to channel {channel_id}")
                else:
                    logger.error(f"Could not find channel {channel_id} for audiobook delivery.")
            else:
                channel = self.bot.get_channel(int(channel_id))
                if channel:
                    await channel.send(f"❌ **Audiobook production failed for '{title}'**")
                    
        except Exception as e:
            logger.error(f"Background audiobook production failed: {e}")
            try:
                channel = self.bot.get_channel(int(channel_id))
                if channel:
                    await channel.send(f"❌ **Audiobook Production Failed**: {str(e)[:500]}")
            except Exception:
                pass
