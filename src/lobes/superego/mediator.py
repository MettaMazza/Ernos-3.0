...
class MediatorAbility(BaseAbility):
    ...
    async def arbitrate(self, user_claim: Dict[str, str], core_fact: Dict[str, Any],
        ...
        try:
            ...
            template = load_prompt("src/prompts/mediator_prompt.txt")
        ...
        prov = core_fact.get("provenance") or {}
    ...
    async def _execute_verdict(self, verdict_data: Dict, user_claim: Dict,
        ...
        if not kg:
            logger.info("No KG available for mediator execution")
            return "no_kg_available"

        # Helper: invalidate stale vector entries referencing the old fact
        def _invalidate_stale_vectors(old_subject: str, old_object: str, reason: str):
            try:
                vs = self.bot.hippocampus.vector_store
                keywords = [k for k in [old_subject, old_object] if k]
                if keywords:
                    count = vs.invalidate_by_content(
                        keywords=keywords,
                        reason=reason
                    )
                    if count:
                        logger.info(f"Mediator: Invalidated {count} stale vector entries ({reason})")
            except Exception as e:
                logger.warning(f"Vector invalidation after verdict failed: {e}")
        ...
        if verdict == "ACCEPT":
            ...
            try:
                ...
                # Invalidate vector entries referencing the OLD (now-overridden) fact
                _invalidate_stale_vectors(
                    core_fact.get("subject", ""),
                    core_fact.get("object", ""),
                    f"mediator_accept: {user_claim.get('subject', '')} "
                    f"{user_claim.get('predicate', '')} {user_claim.get('object', '')}"
                )
            ...
            try:
                ...
                # Invalidate vector entries that may assert the old single-truth
                # (the fact is now contested, so old absolute statements are stale)
                _invalidate_stale_vectors(
                    core_fact.get("subject", ""),
                    core_fact.get("object", ""),
                    f"mediator_annotate: claim contested by user {user_id}"
                )
