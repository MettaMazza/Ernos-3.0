...
class ErnosBot(commands.Bot):
    def __init__(self):
        ...
        intents.reactions = True  # Required for Garden on_raw_reaction_add
        ...
        try:
            ...
            logger.warning(f"SkillForge failed to initialize: {e}")
    ...
    async def setup_hook(self):
        ...
        await self.load_extension("src.bot.cogs.skill_commands")
        await self.load_extension("src.bot.cogs.mode_commands")
        await self.load_extension("src.bot.cogs.garden")
        ...
        skills_dir = data_dir() / "core" / "skills"
        loaded = self.skill_registry.load_skills(skills_dir)
        logger.info(f"Loaded {loaded} core skill(s) from {skills_dir}")
        ...
        if users_dir.exists():
            for user_folder in users_dir.iterdir():
                if user_folder.is_dir():
                    user_skills = user_folder / "skills"
                    if user_skills.exists():
                        count = self.skill_registry.load_skills(user_skills, user_id=user_folder.name)
                        if count > 0:
                            logger.info(f"Loaded {count} skill(s) for user {user_folder.name}")
        ...
        # Ensure skill_bridge is imported to register 'execute_skill' tool
        try:
            import src.tools.skill_bridge
            logger.info("Skill Bridge initialized (execute_skill registered).")
        except Exception as e:
            logger.error(f"Failed to initialize Skill Bridge: {e}")
        ...
        try:
            ...
            try:
                from src.tools.skill_admin_tools import restore_schedules
                await restore_schedules()
            except ImportError:
                logger.warning("Could not import restore_schedules (skill tools not loaded?)")
            except Exception as e:
                logger.error(f"Failed to restore schedules: {e}")

            logger.info("All schedulers started (backup + dream consolidation + persistent skills)")
        except Exception as e:
        ...
        try:
            ...
            admin_personas = data_dir() / "users/1299810741984956449/personas"
    ...
    async def close(self):
        ...
        try:
            ...
            temporal = TemporalTracker(register_boot=True)
