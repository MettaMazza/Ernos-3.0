...
async def process_non_image_attachments(
    ...
    for attachment in message.attachments:
        ...
        attachment_origin = await _check_attachment_provenance(attachment, message)
        attachment_origin_tags.append(attachment_origin)

        # Handle JSON backup files
        if attachment.filename.endswith(".json"):
            result = await _process_json_backup(attachment)
        ...
        # Handle documents (PDF, DOCX, TXT, Code, Config, etc.)
            ...
            doc_ctx = await _extract_and_inject_document(
                attachment, attachment_origin, engine, message
            )
            if doc_ctx:
...
async def _check_attachment_provenance(attachment, message):
    """Run provenance check on a non-image attachment."""
    try:
        from src.security.provenance import ProvenanceManager
...
async def _process_json_backup(attachment):
    """Classify a JSON attachment as master/valid/legacy backup or ignore."""
    try:
...
async def _extract_and_inject_document(attachment, attachment_origin, engine, message):
    """Extract text from a document attachment and return context injection string."""
    try:
        from .chat_helpers import AttachmentProcessor
        text_content = await AttachmentProcessor.extract_text(attachment)
        if not text_content:
            return None
