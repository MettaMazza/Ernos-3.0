...
class AdminEngine(commands.Cog):
    ...
    @commands.hybrid_command(name="botstoggle", description="Toggle whether Ernos listens to other bots")
    async def toggle_bots(self, ctx):
        """Toggle bot listening on/off. When on, Ernos reads and replies to other bots."""
        settings.LISTEN_TO_BOTS = not settings.LISTEN_TO_BOTS
        state = "🤖 **ON** — Listening to other bots" if settings.LISTEN_TO_BOTS else "🔇 **OFF** — Ignoring other bots"
        logger.info(f"Bot listening toggled to {settings.LISTEN_TO_BOTS} by {ctx.author.id}")
        await ctx.send(f"Bot Listening: {state}", ephemeral=True)
