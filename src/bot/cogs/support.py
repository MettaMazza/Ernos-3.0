...
class SupportCog(commands.Cog):
    ...
    async def generate_support_response(self, thread, user, user_input, is_new=False):
        ...
        try:
            ...
            identity_path = Path("src/prompts/identity_core.txt")
            manual_path = Path("src/prompts/user_manual.txt")
            
            identity_text = identity_path.read_text(encoding="utf-8") if identity_path.exists() else "You are Ernos."
            manual_text = manual_path.read_text(encoding="utf-8") if manual_path.exists() else "No manual found."
