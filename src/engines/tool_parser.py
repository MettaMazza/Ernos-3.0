...
logger = logging.getLogger("Engine.ToolParser")
...
def parse_tool_args(args_str: str) -> dict:
    ...
    while i < n:
        ...
        while i < n and s[i] not in '= \t':
            i += 1
        key = s[key_start:i].strip()  # type: ignore
        if not key:
            break
    ...
    if result:
        logger.debug(f"Parsed {len(result)} tool args: {list(result.keys())}")
        return result

    # Last resort: treat entire string as content
    logger.warning(f"Could not parse tool args, treating as single content param")
