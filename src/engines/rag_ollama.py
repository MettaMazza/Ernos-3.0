    ...
    def generate_response(self, prompt: str, context: any = None, system_prompt: str = None, images: list[bytes] = None) -> str:
        ...
        if total_len > limit:
            ...
            if total_len > limit:
                 ...
                 available = max(limit - sys_len - 1000, 1000)
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Diagnostic logging
                logger.info(f"Ollama generate result: len={len(result)}, prompt_len={len(full_prompt)}, "
                            f"model={self._model}, eval_count={response.get('eval_count', '?')}, "
                            f"done_reason={response.get('done_reason', '?')}")
                if not result:
                    logger.warning(f"Ollama returned empty response. Full response keys: {list(response.keys())}")
                    # Dump for debugging
                    try:
                        with open("memory/debug_empty_response.txt", "w") as f:
                            f.write(f"RESPONSE OBJECT:\n{response}\n\nPROMPT (first 2000 chars):\n{full_prompt[:2000]}")
                    except Exception:
                        pass
                return result
            except Exception as e:
                is_transient = any(code in str(e) for code in ["500", "502", "503", "timed out"])
