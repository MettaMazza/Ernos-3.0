            ...
            if forced_response and not tool_pattern.search(forced_response):
                ...
                try:
                    ...
                    if skeptic and not skip_defenses:
                        logger.info(f"[Regen {retry_count}] Skeptic Audit starting on forced response")
                        ...
                        if not audit_res["allowed"]:
                            ...
                            audit_fail_count += 1
                            logger.warning(f"[Regen {retry_count}] Skeptic BLOCKED forced response (audit_fail {audit_fail_count}/{MAX_AUDIT_RETRIES}): {last_block_reason}")
                            blocked = True
                        else:
                            logger.info(f"[Regen {retry_count}] Skeptic Audit PASSED")
                except Exception as e:
                    logger.error(f"Skeptic audit on forced retry failed: {e}")

                if not blocked:
                    final_response_text = forced_response
                    logger.info(f"[Regen {retry_count}] Forced final generation ACCEPTED")
            else:
...
def strip_output_artifacts(final_response_text: str, files_to_upload: list, send_thought_to_mind_fn=None) -> str:
    ...
    """
    # Strip file path references and esoteric Tape tags
    ...
    if final_response_text:
        # Ernos natively outputs `<EMIT: ...>` under the Tape architecture
        final_response_text = re.sub(r'<EMIT:\s*(.*?)\s*>', r'\1', final_response_text, flags=re.DOTALL)
