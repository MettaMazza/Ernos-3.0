...
class SkillSandbox:
    ...
    def check_permissions(
        ...
        # 2. Tool Whitelist (Admin-authored skills bypass this)
        admin_id = str(getattr(settings, 'ADMIN_ID', ''))
        admin_ids = [str(i) for i in getattr(settings, 'ADMIN_IDS', [])]
        is_admin_skill = str(skill.author) == admin_id or str(skill.author) in admin_ids
        
        if requested_tools and not is_admin_skill:
        ...
        # 4. Turn Limit (One Skill Per Turn)
        # We track the last time a skill was executed. If it was very recent (< 2 seconds),
        # we block it to prevent the LLM from looping multiple skills in one generation.
        # REALITY CHECK: The LLM generates the tool call, then the system executes it.
        # If the LLM generates 5 tool calls in a row, they might all hit this within milliseconds.
        # Or if it's a re-entry loop.
        
        last_exec_time = 0
        if self._rate_tracker[user_id]:
            last_exec_time = self._rate_tracker[user_id][-1]
            
        # If less than 5 seconds since last skill, block it.
        # This effectively limits to 1 pending skill per "turn" or thought block.
        if (now - last_exec_time) < 5.0:
             reason = "Rate limit denied: only one skill execution allowed per 5 seconds (Loop Prevention)."
             logger.warning(f"SECURITY: {reason}")
             return False, reason
    ...
    def execute(
        ...
        # Admin-authored skills get full tool access
        admin_id = str(getattr(settings, 'ADMIN_ID', ''))
        admin_ids = [str(i) for i in getattr(settings, 'ADMIN_IDS', [])]
        is_admin_skill = str(skill.author) == admin_id or str(skill.author) in admin_ids
        ...
        if is_admin_skill:
            # Admin bypass: full tool access
            instructions_block = (
                f"[SKILL EXECUTION: {skill.name} v{skill.version}]\n"
                f"[AUTHOR: {skill.author} (ADMIN)]\n"
                f"[ALLOWED TOOLS: ALL — Admin-authored skill has unrestricted tool access]\n"
                f"[CONTEXT: {context}]\n\n"
                f"INSTRUCTIONS:\n{skill.instructions}\n\n"
                f"[END SKILL: {skill.name}]\n"
                f"You have FULL tool access for this admin-authored skill. "
                f"Use any tools needed to complete the instructions above.\n\n"
                f"[SKILL_BOUNDARY_START]\n"
                f"ACTIVE_SKILL={skill.name}\n"
                f"ALLOWED_TOOLS=ALL\n"
                f"ADMIN_AUTHORED=TRUE\n"
