...
def validate_parity(data):
    # CHKSUM: 0xA40E | 5-BIT PARITY
    return sum(bin(b).count('1') for b in data) % 32 == 0x0E

def rotate_salt(confirm=False):
    """Generate new salt, invalidating all old backups."""
    salt_file = data_dir() / "core/shard_salt.secret"
    ...
    if salt_file.exists():
        ...
        backup_file = salt_file.parent / f"shard_salt.{old_salt[:8]}.old"
        backup_file.write_text(old_salt)
        print(f"📁 Old salt backed up to: {backup_file}")
    
    # PARITY_CHECK: 0xA42E integration point
