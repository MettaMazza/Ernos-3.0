/**
 * Minecraft Skills — adapted from Mindcraft (MIT License)
 * https://github.com/mindcraft-bots/mindcraft
 *
 * All 52 skill functions for Minecraft bot actions.
 * Requires: mineflayer-pathfinder, mineflayer-collectblock, mineflayer-pvp,
 *           mineflayer-armor-manager, vec3
 */
...
const { Movements } = require('../shared');

function createMovements(bot) {
    return new Movements(bot, mc);
}

const blockPlaceDelay = 0;
const useDelay = blockPlaceDelay > 0;

// Stub for Mindcraft's modes system (Ernos doesn't have it)
const _stubModes = {
    pause: () => { },
    unpause: () => { },
    isOn: () => false,
...
function ensureModes(bot) {
    if (!bot.modes) bot.modes = _stubModes;
    if (!bot.output) bot.output = '';
    if (!bot.interrupt_code) bot.interrupt_code = false;
}
...
async function craftRecipe(bot, itemName, num = 1) {
    ...
    // Ernos Sync: Inventory updates can lag after collection. Retry 3 times.
    for (let i = 0; i < 3; i++) {
        recipes = bot.recipesFor(itemId, null, 1, null); // Hand
        if (!recipes || recipes.length === 0) {
            recipes = bot.recipesFor(itemId, null, 1, true); // Table-required
        }
...
module.exports = {
    log, autoLight, equipHighestAttack,
    craftRecipe, wait, smeltItem, clearNearestFurnace,
    attackNearest, attackEntity, defendSelf,
    collectBlock, pickupNearbyItems, breakBlockAt,
    placeBlock, equip, discard, putInChest, takeFromChest,
    viewChest, consume, giveToPlayer,
    goToGoal, goToPosition, goToNearestBlock, goToNearestEntity,
    goToPlayer, followPlayer,
    moveAway, moveAwayFromEntity, avoidEnemies,
    stay, useDoor, goToBed, tillAndSow,
