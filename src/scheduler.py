...
class TaskScheduler:
    ...
    async def _check_and_run(self, catch_up: bool = False):
        ...
        for name, task in self._tasks.items():
            ...
            # Catch-up mode: Run if current time is PAST scheduled time and NOT run today
            is_time_match = now.hour == task["hour"] and now.minute == task["minute"]
            is_past_time = now.hour > task["hour"] or (now.hour == task["hour"] and now.minute > task["minute"])

            if is_time_match or (catch_up and is_past_time):
    ...
    async def _scheduler_loop(self):
        """Main scheduler loop, synchronized to minute boundaries."""
        while self._running:
            await self._check_and_run()
            # Calculate sleep time to hit the next minute boundary exactly
            now = datetime.now()
    ...
    def start(self):
        ...
        asyncio.create_task(self._check_and_run(catch_up=True))
