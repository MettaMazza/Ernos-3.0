...
class BackupManager:
    ...
    async def daily_backup(self) -> str:
        ...
        try:
            ...
            # Backup core
            if Path("memory/core").exists():
                shutil.copytree("memory/core", backup_path / "core", dirs_exist_ok=True)
                
            # Backup users
            if Path("memory/users").exists():
                shutil.copytree("memory/users", backup_path / "users", dirs_exist_ok=True)
