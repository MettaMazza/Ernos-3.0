...
class PromptManager:
    ...
    def __init__(self, prompt_dir: str = "./src/prompts"):
        ...
        self.kernel_file = os.path.join(prompt_dir, "kernel_backup.txt")
        self.architecture_file = os.path.join(prompt_dir, "dynamic_context.txt")
        self.identity_file = os.path.join(prompt_dir, "identity_core.txt")
        ...
        self.identity_professional_file = os.path.join(prompt_dir, "identity_professional.txt")
    
    def _read_file(self, filepath: str) -> str:
        """Read a prompt file, transparently decrypting if encrypted."""
        if not filepath:
            return ""
        try:
    ...
    def _generate_tool_manifest(self) -> str:
        ...
        try:
            ...
            manifest_lines = [
                "## AVAILABLE TOOLS",
                "You have access to the following tools. To execute a tool, you MUST use the following exact syntax on its own line:",
                "`[TOOL: tool_name(arg_name=\"value\", other_arg=123)]`",
                "You can execute multiple tools in parallel by emitting multiple `[TOOL: ...]` blocks.",
                "DO NOT narrate that you are going to use a tool. Just output the `[TOOL: ...]` block.\n"
            ]
            for tool in tools:
                name = tool.name
    ...
    def get_system_prompt(self, 
        ...
        try:
            ...
            pass  # If no template variables or malformed braces, leave as-is

        # Inject Active Model & Config (Safe Replace)
        model_name = active_engine if (active_engine and active_engine != "Unknown") else "Standard Model"
        
        architecture = architecture.replace("{active_model}", model_name)
        architecture = architecture.replace("{active_mode}", active_mode)
        
        kernel = kernel.replace("{active_model}", model_name)
        kernel = kernel.replace("{active_mode}", active_mode)
        kernel = kernel.replace("{model_config}", model_config)
        ...
        if not identity:
            # Professional mode: PRIVATE scope only, no persona, no custom identity
            if interaction_mode == "professional" and scope == "PRIVATE" and not is_persona_mode:
                identity = self._read_file(self.identity_professional_file)
                logger.info(f"Loaded PROFESSIONAL identity for user {user_id}")
            if not identity:
        ...
        fork_defaults = {
            ...
            "time_since_last_message": "No prior messages.",
        }
        ...
        parts = []
        # ─── Append dynamic context HUD + identity to kernel ────────────
        # The kernel is the single ground truth for all internal systems
        # (superego, skeptic, etc). It now contains:
        #   Laws (kernel_backup.txt)
        #   + Perception (formatted dynamic HUD)
        #   + Soul (identity — core, professional, or persona)
        if formatted_dynamic:
            kernel += f"\n\n{formatted_dynamic}"
        if identity:
            kernel += f"\n\n{identity}"
