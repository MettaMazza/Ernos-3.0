...
class UnifiedPreProcessor(BaseAgent):
    ...
    CRITICAL INSTRUCTION: Analyze the USER_INPUT in context. Return ONLY a JSON object. Do NOT output any standard text, thoughts, or tool calls. Do NOT use the [TOOL: name(args)] syntax. ONLY output RAW JSON.
    ...
    async def process(self, user_input: str, context: str = "", has_images: bool = False, attachment_info: str = "", images: list = None, system_context: str = "") -> dict:
        ...
        try:
            ...
            if analysis:
                ...
                logger.warning(f"PreProcessor returned non-JSON: {response[:100]}...")
                # If the agent just said something conversational without formatting,
                # wrap it in a fallback structure that indicates conversational intent.
                return {
                    ...
                    "justification": f"Fallback extraction. Raw output: {response[:30]}"
                }
