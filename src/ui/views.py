...
class ResponseFeedbackView(discord.ui.View):
    ...
    async def like_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            logger.debug("Like interaction expired (token >3s old) — feedback still logged")
        # Log positive RLHF regardless of whether the ack succeeded
    ...
    async def dislike_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            logger.debug("Dislike interaction expired (token >3s old) — feedback still logged")
        # Log negative RLHF regardless of whether the ack succeeded
    ...
    def _log_feedback(self, user_id: int, sentiment: str, response_text: str):
        ...
        feedback_path = data_dir() / "core/rlhf_feedback.jsonl"
    ...
    async def tts_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not hasattr(self.bot, 'voice_manager') or not self.bot.voice_manager:
             try:
                 ...
                 logger.debug("TTS interaction expired")
             return
        ...
        if self.audio_msg:
            try:
                ...
                logger.debug("Toggle interaction expired or message not found")
            self.audio_msg = None
            return
        ...
        try:
            ...
            logger.debug("TTS defer interaction expired")
            return
