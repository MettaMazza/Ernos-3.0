    ...
    try:
        ...
        if mode == "append":
            ...
            if target not in existing:
                # Failsafe: if target not found but content is substantial,
                # the caller likely wants a full overwrite (common AI mistake)
                if len(content) > 200:
                    logger.warning(f"surgical_edit: replace target not found, auto-falling back to overwrite (content len={len(content)})")
                    new_content = content
                    mode = "overwrite (auto-fallback)"
                else:
