...
@ToolRegistry.register(
    ...
    description=(
        ...
        "Args: target (markdown string or URL).\n"
        "      is_url (bool, default False) - SET TO TRUE ONLY IF target IS A VALID HTTP/HTTPS URL. "
        "DO NOT SET THIS TO TRUE IF YOU ARE PASSING MARKDOWN TEXT! "
...
async def generate_pdf(
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Protection against hallucinated `is_url=True` with markdown text
            if is_url and (len(target) > 500 or target.strip().startswith("#") or "\n" in target or not target.startswith("http")):
                logger.warning("LLM hallucinated is_url=True but passed a markdown block. Overriding to False.")
                is_url = False
        ...
        ProvenanceManager.log_artifact(output_path, "pdf", {
            ...
            "scope": request_scope, "theme": theme, "intention": intention
        })
...
async def preview_document(pdf_path: str, **kwargs) -> str:
    ...
    try:
        ...
        # Method 1: PyMuPDF (fitz) — direct PDF→PNG, no browser needed
        try:
            import fitz  # PyMuPDF
            ...
            doc.close()
            logger.info(f"Preview generated (PyMuPDF): {preview_path}")
            return (
            ...
            logger.info("PyMuPDF not available, falling back to Playwright HTML viewer")

        # Method 2: Playwright fallback — render PDF via pdf.js viewer (not file:// goto)
        from playwright.async_api import async_playwright
        import base64
        ...
        # Embed PDF in an HTML page using pdf.js for rendering
        html_viewer = f"""<!DOCTYPE html>
<html><head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"></script>
<style>body {{ margin:0; background:#fff; }} canvas {{ width:100%; }}</style>
</head><body>
...
pdfjsLib.getDocument({{data: bytes}}).promise.then(pdf => {{
    pdf.getPage(1).then(page => {{
        ...
        async with async_playwright() as p:
            ...
            await page.set_content(html_viewer, wait_until="networkidle")
            await page.wait_for_timeout(3000)  # Let pdf.js render
        ...
        logger.info(f"Preview generated (pdf.js): {preview_path}")
        return (
            f"Preview image saved: {preview_path}\n"
            "Examine this image to check visual quality (typography, contrast, layout, spacing). "
            "If issues found, adjust the content/theme and re-render."
