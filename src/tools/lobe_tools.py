...
@ToolRegistry.register(name="generate_video", description="Create an AI video.")
...
@ToolRegistry.register(
    name="generate_music",
    description=(
        "Generate AI music from a text prompt using MusicGen Large. "
        "Describe genre, mood, instruments, tempo, and style. Supports full songs via segment chaining. "
        "Args: prompt (str — e.g. 'lo-fi hip hop with mellow piano and vinyl crackle'), "
        "duration (int — seconds, default 10, up to 300 for full songs), "
        "intention (str — why you are creating this music)."
    ),
)
async def generate_music(prompt: str, duration: int = 10, intention: str = None, **kwargs) -> str:
    ...
    return await ability.execute(
        ...
        channel_id=kwargs.get("channel_id"),
        intention=intention,
        duration=min(max(duration, 1), 300),  # Clamp 1-300s
    )
...
@ToolRegistry.register(
    ...
    description=(
        ...
        "This is a production pipeline that combines three engines: "
        "Kokoro (Ernos narrator voice), Qwen3-TTS (unique character voices), "
        "and MusicGen (background music and sound effects). "
        "SCRIPT FORMAT — use these markers:\n"
        "[NARRATE] Narration text here.\n"
        "[VOICE: \"CharName\" | voice description for Qwen3-TTS]\n"
        "\"Dialogue spoken by CharName.\"\n"
        "[MUSIC: music prompt, duration in seconds]\n"
        "[SFX: sound effect description, duration in seconds]\n"
        "[PAUSE: seconds]\n"
        "VOICE CONSISTENCY: Each character's voice is generated once via VoiceDesign, "
        "then cloned for all subsequent lines — ensuring consistent voices throughout. "
        "Args: script (str — the full marked-up script), "
        "title (str — audiobook title for logging), "
        "intention (str — why creating this audiobook)."
    ),
)
