...
PARAM_ALIASES = {
    ...
    # Skill forge variants
    "prompt": "instructions",    # propose_skill
    "sop": "instructions",      # propose_skill
    "steps": "instructions",    # propose_skill
    "procedure": "instructions", # propose_skill
}
...
class ToolRegistry:
    ...
    @classmethod
    def register(cls, name: str = None, description: str = None, parameters: Dict[str, str] = None):
        ...
        def decorator(func):
            ...
            # If explicit parameters are provided (descriptions), use them.
            # This allows providing helpful descriptions instead of just types.
            final_params = detected_params.copy()
            if parameters:
    ...
    async def execute(cls, tool_name: str, *args, request_scope=None, user_id=None, bot=None, channel=None, **kwargs):
        ...
        # Inject Turn ID (for per-turn deduplication)
        # turn_id flows through kwargs naturally if the tool accepts it.
        # Only inject if it was passed as a top-level kwarg to execute() but
        # wasn't already provided to the tool by the caller.
        if 'turn_id' in kwargs:
            # If the tool doesn't accept turn_id, strip it to prevent errors
            if 'turn_id' not in params and not has_var_keyword:
                kwargs.pop('turn_id')
            # Otherwise leave it in kwargs — it will be passed through naturally
...
import src.tools.skill_forge_tool
import src.tools.agent_tools  # Multi-agent orchestration tools
import src.tools.self_stop_tool  # Self-abort tool
import src.tools.tape_tools  # 3D Tape operations intercepted by Cognition Engine
import src.tools.web  # Web search, browse, download_file
