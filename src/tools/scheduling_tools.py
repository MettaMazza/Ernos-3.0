...
@ToolRegistry.register(
    ...
    description="Check the current system time and timezone."
)
async def check_current_time(**kwargs) -> str:
    """Returns the current date and time (UTC/Local)."""
    ...
    return f"🕒 Current Time: {now.strftime('%Y-%m-%d %H:%M:%S')} ({settings.TIMEZONE})"
...
async def manage_calendar(
    ...
    **kwargs
    ...
    """
    Manage calendar events with full scoping.
    ...
    Scopes: CORE (system), CORE_PUBLIC (shared), PRIVATE (user DM), PUBLIC (user public)
    """
    from src.memory.calendar import CalendarManager
    
    scope_map = {
        "CORE": PrivacyScope.CORE_PRIVATE,
        "CORE_PRIVATE": PrivacyScope.CORE_PRIVATE,
        "CORE_PUBLIC": PrivacyScope.CORE_PUBLIC,
        "PRIVATE": PrivacyScope.PRIVATE,
        "PUBLIC": PrivacyScope.PUBLIC
    }
    
    # Use the request_scope from context if no explicit scope provided
    if scope == "PRIVATE" and kwargs.get("request_scope"):
        scope = kwargs["request_scope"]
