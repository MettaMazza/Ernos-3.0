...
4. Lesson extraction
5. KG node pruning
6. KG connection discovery (Gardener)
7. Quarantine processing (re-parenting orphaned KG entries)
8. Immune cache persistence
...
STATUS_FILE = data_dir() / "system/dream_status.json"
...
class DreamConsolidationDaemon:
    ...
    6. Discovers and creates missing KG connections
    7. Persists sentinel cache to disk
    """
    ...
    async def run(self):
        ...
        try:
            ...
            # 3.5. KG Connection Discovery — wire under-connected nodes
            connected = await self._connect_kg_nodes()
            results.append(f"KG connected: {connected} new relationships")
    ...
    async def _prune_kg_nodes(self) -> int:
        """Prune orphaned KG nodes (zero relationships, 30+ days old)."""
        ...
        try:
            result = graph.prune_orphan_nodes(min_age_days=30)
            pruned = result.get("pruned", 0)
            skipped = result.get("skipped_immutable", 0)
            if pruned > 0 or skipped > 0:
                logger.info(f"KG pruned {pruned} orphan nodes (skipped {skipped} immutable)")
            return pruned
        except Exception as e:
            logger.warning(f"KG prune failed (non-critical): {e}")
            return 0

    async def _connect_kg_nodes(self) -> int:
        """
        Run KG connection discovery via GardenerAbility.
        Finds under-connected nodes and creates missing relationships.
        """
        if not hasattr(self.bot, 'cerebrum') or not self.bot.cerebrum:
            return 0

        try:
            strategy_lobe = self.bot.cerebrum.get_lobe("StrategyLobe")
            if not strategy_lobe:
                logger.warning("KG connect: StrategyLobe not found")
                return 0

            gardener = strategy_lobe.get_ability("GardenerAbility")
            if not gardener:
                logger.warning("KG connect: GardenerAbility not found")
                return 0

            report = await gardener.connect_graph()
            logger.info(f"KG connection discovery result: {report[:200]}")

            # Parse count from report
            import re as _re
            match = _re.search(r"New Connections Stored\*\*:\s*(\d+)", report)
            if match:
                return int(match.group(1))
            return 0

        except Exception as e:
            logger.warning(f"KG connection discovery failed (non-critical): {e}")
            return 0
    ...
    def _infer_user_id(self, entry: dict, user_lookup: dict):
        ...
        # 4.5 Strategy 6: Contextual Graph Lookup (Cross-Relationship)
        # Check if either entity has a dominant owner in the graph
        if hasattr(self.bot, 'hippocampus') and self.bot.hippocampus:
            graph = self.bot.hippocampus.graph
            if graph:
                for text in [source, target]:
                    graph_owner = graph.get_dominant_owner(text)
        ...
        if "user_id" in violation or "identity" in violation or "ownership" in violation:
            if len(user_lookup) == 1:
                uid = list(user_lookup.values())[0]
                # Strategy 5 Hardening: Verify user actually exists in the graph
                if hasattr(self.bot, 'hippocampus') and self.bot.hippocampus:
                    graph = self.bot.hippocampus.graph
                    if graph and graph.check_user_has_data(uid):
                        return uid
    ...
    def _persist_sentinel_cache(self) -> bool:
        ...
        try:
            ...
            cache_path = data_dir() / "system/sentinel_cache.json"
