...
**Subsystem**: Action Registry, DarwinGodal System, & Feedback Manager
**Goal**: Execution, System Mutation, and Reinforcement Learning.

*   **Action Registry**: Logs execution of standard tools (e.g., file edits, web searches) in JSON `[TOOL: name(args)]` format.
*   **DarwinGodal System**: The overarching code evolution system enabling Ernos to modify its own source code, optimize prompts, and forge new skills autonomously in a sandboxed, iterative manner.
*   **3D Turing Tape Engine**: Specialized coordinate-based cognitive execution, driving the DarwinGodal system. Ernos uses standard JSON tools (e.g., `tape_seek`, `tape_insert`, `tape_edit_code`) to navigate a 3-dimensional logical grid, manipulating internal system tracks, working memory, or the source code natively without losing high-level context.
