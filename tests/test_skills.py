...
# MIGRATED to tests/skills/test_skill_core.py
class TestSkillLoader_DEPRECATED:
...
""")
    ...
    def test_registry_thread_safety(self):
        ...
        def worker(i):
            skill = SkillDefinition(name=f"thread_skill_{i}", description="...", instructions="...", scope="PUBLIC")
...
class TestSkillSandbox:
    def _make_skill(self, allowed_tools=None, scope="PUBLIC"):
        return SkillDefinition(
            name="test_skill",
            description="Test skill",
            instructions="Test",
            allowed_tools=allowed_tools or ["search_web", "save_core_memory"],
            scope=scope,
            version="1.0.0",
            author="system"
        )
    ...
    @pytest.mark.asyncio
    async def test_audit_log_async(self):
        sandbox = SkillSandbox()
        skill = self._make_skill()
        sandbox.execute(skill, context="", user_id="123", scope="PUBLIC")
        log = sandbox.get_audit_log()
