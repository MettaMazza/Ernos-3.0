...
class TestWeeklyQuota(unittest.TestCase):
    ...
    def test_assign_task_on_work_day(self, mock_now, mock_day, mock_iso):
        ...
        from src.tools.weekly_quota import assign_dev_task, start_work_session, _load_week
        start_work_session(plan="Unit Test Plan", predicted_tasks=5)
    ...
    def test_assign_task_on_saturday(self, mock_now, mock_day, mock_iso):
        ...
        from src.tools.weekly_quota import assign_dev_task, start_work_session
        start_work_session(plan="Saturday work", predicted_tasks=5)
    ...
    def test_assign_task_quota_full(self, mock_now, mock_day, mock_iso):
        ...
        from src.tools.weekly_quota import assign_dev_task, _load_week, _save_week, start_work_session
        # Pre-fill quota
        state = _load_week()
        state["days"]["monday"]["hours_logged"] = 3.0
        # Create active session manually or via tool
        state["days"]["monday"]["session"] = {
            "status": "ACTIVE", "plan": "Test", "predicted_cap": 5, "started_at": datetime(2026, 2, 9, 9, 0).isoformat()
        }
        _save_week(state)
    ...
    def test_complete_task(self, mock_now, mock_day, mock_iso):
        ...
        from src.tools.weekly_quota import assign_dev_task, complete_dev_task, _load_week, start_work_session, _day_name, _save_week
        mock_now.return_value = datetime(2026, 2, 10, 14, 0, 0)
        start_work_session(plan="Test completion", predicted_tasks=3)
        assign_dev_task(description="Test task", estimated_hours=1.0)
        
        # Manually inject passing verification
        state = _load_week()
        today = _day_name()
        state["days"][today]["verifications"] = {"test.py": {"status": "PASSED"}}
        _save_week(state)
    ...
    def test_complete_task_on_saturday(self, mock_now, mock_day, mock_iso):
        from src.tools.weekly_quota import assign_dev_task, complete_dev_task, start_work_session, verify_staging_item
        mock_now.return_value = datetime(2026, 2, 14, 10, 0, 0)
        start_work_session(plan="Saturday completion", predicted_tasks=3)
        assign_dev_task(description="Saturday task")
        # QA gate requires output_files and verification
        with patch("src.tools.weekly_quota.subprocess.run") as mock_sub:
            mock_res = MagicMock()
            mock_res.returncode = 0
            mock_res.stdout = "OK"
            mock_res.stderr = ""
            mock_sub.return_value = mock_res
            verify_staging_item(path="output.md", test_command="echo ok")
        mock_now.return_value = datetime(2026, 2, 14, 12, 0, 0)
        result = complete_dev_task(output_files="output.md", summary="Done")
