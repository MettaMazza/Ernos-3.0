...
class TestGetPrerequisites:
    ...
    def test_simple_craft(self):
        """Planks need log."""
        result = get_prerequisites("planks")
        assert ("craft", "planks") in result
        # Should include log as prerequisite
        log_entries = [(item, count) for item, count in result if item == "log"]
...
class TestCanCraft:
    def test_can_craft_planks(self):
        inv = {"log": 1}
        assert can_craft("planks", inv) is True

    def test_cannot_craft_planks_no_log(self):
        inv = {}
        assert can_craft("planks", inv) is False
...
class TestMissingIngredients:
    def test_all_missing(self):
        result = missing_ingredients("planks", {})
    ...
    def test_nothing_missing(self):
        result = missing_ingredients("planks", {"log": 5})
        assert result == []
