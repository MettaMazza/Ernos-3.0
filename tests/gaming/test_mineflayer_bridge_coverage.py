...
class TestBridgeHighLevelCommands:
    ...
    async def test_goto(self, bridge):
        ...
        bridge.execute.assert_called_with("goto", {"x": 10, "y": 64, "z": 20, "range": 2})

    @pytest.mark.asyncio
    async def test_follow(self, bridge):
        await bridge.follow("player1", range=5)
        bridge.execute.assert_called_with("follow", {"username": "player1", "range": 5})
    ...
    async def test_attack(self, bridge):
        ...
        bridge.execute.assert_called_with("attack", {"entity_type": "hostile"})

    @pytest.mark.asyncio
    async def test_craft(self, bridge):
        await bridge.craft("planks", 4)
        bridge.execute.assert_called_with("craft", {"item": "planks", "count": 4})
    ...
    async def test_protect_without_coords(self, bridge):
        ...
        bridge.execute.assert_called_with("protect", {"username": "user1", "radius": 50})

    @pytest.mark.asyncio
    async def test_protect_with_coords(self, bridge):
        await bridge.protect("user1", 50, x=10, y=64, z=20)
        bridge.execute.assert_called_with("protect", {"username": "user1", "radius": 50, "x": 10, "y": 64, "z": 20})
    ...
    async def test_plant(self, bridge):
        ...
        bridge.execute.assert_called_with("plant", {"seed": "carrot", "count": 3})
    ...
    async def test_find_without_go(self, bridge):
        ...
        bridge.execute.assert_called_with("find", {"block": "diamond_ore", "go": False, "radius": 64}, timeout=10)

    @pytest.mark.asyncio
    async def test_find_with_go(self, bridge):
        await bridge.find("diamond_ore", go=True)
        bridge.execute.assert_called_with("find", {"block": "diamond_ore", "go": True, "radius": 64}, timeout=60)
