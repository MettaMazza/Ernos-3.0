...
class TestObserve:
    ...
    async def test_observe_screenshot_timeout(self):
        ...
        agent.bridge.execute.return_value = BridgeResponse(True, {
            ...
            "nearby_entities": [], "hostiles_nearby": False, "is_day": True
        })
...
class TestBuildReflexes:
    def test_basic_reflexes(self):
        ...
        chain = agent._build_safety_reflexes(state)
        # Should have precog_action commands + collect_drops
        assert len(chain) >= 3
        commands = [c["command"] for c in chain]
        assert "precog_action" in commands
...
class TestUnstuck:
    @pytest.mark.asyncio
    async def test_unstuck_mild_jump(self):
        """Counter <= 3: should jump and explore."""
        agent = make_perception_agent()
        agent._stuck_counter = 3
    ...
    @pytest.mark.asyncio
    async def test_unstuck_medium_dig(self):
        """Counter 4-5: should dig forward."""
        agent = make_perception_agent()
    ...
    @pytest.mark.asyncio
    async def test_unstuck_severe_relocation(self):
        """Counter > 5: should relocate far away to surface."""
        agent = make_perception_agent()
        agent._stuck_counter = 7
        ...
        with patch("src.gaming.perception.log_embodiment"):
            result = await agent._unstuck()
        assert result == "explore"
        # Should have called goto for relocation
        agent.bridge.goto.assert_called_once()
        # Verify Y was clamped to surface (max(50, 64) = 64)
        call_args = agent.bridge.goto.call_args
        assert call_args[0][1] == 64  # y argument

    @pytest.mark.asyncio
    async def test_unstuck_severe_relocation_failure(self):
        """Counter > 5 but get_status fails: should still return explore."""
        agent = make_perception_agent()
        agent._stuck_counter = 7
