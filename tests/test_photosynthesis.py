...
class TestSkillForge:
    ...
    def test_propose_skill(self, tmp_path):
        ...
        with patch.object(SkillForge, 'FORGE_DIR', tmp_path / "forge"), \
            ...
            proposal = forge.propose_skill(
                ...
                action_code="def greet(user): return f'Hello {user}!'",
                reason="Users often say hello"
            )
    ...
    def test_approve_skill(self, tmp_path):
        from src.lobes.strategy.skill_forge import SkillForge
        skills_dir = tmp_path / "skills"
        with patch.object(SkillForge, 'FORGE_DIR', tmp_path / "forge"), \
             patch.object(SkillForge, 'QUEUE_FILE', tmp_path / "forge/pending.json"), \
             patch.object(SkillForge, 'LOG_FILE', tmp_path / "forge/log.json"), \
             patch("src.lobes.strategy.skill_forge.Path", wraps=Path) as mock_path:
            
            forge = SkillForge()
            forge.propose_skill("test_skill", "A test", "test", "print('hi')")
            
            # Mock skill file write directory
            result = forge.approve_skill("test_skill", "admin_001")
    ...
    def test_reject_skill(self, tmp_path):
        ...
        with patch.object(SkillForge, 'FORGE_DIR', tmp_path / "forge"), \
            ...
            forge = SkillForge()
            forge.propose_skill("bad_skill", "Bad", "bad", "bad code")
            result = forge.reject_skill("bad_skill", "admin_001", "unsafe")
            assert result is True
            assert len(forge.get_pending()) == 0
    ...
    def test_forge_summary(self, tmp_path):
        ...
        with patch.object(SkillForge, 'FORGE_DIR', tmp_path / "forge"), \
            ...
            forge = SkillForge()
            forge.propose_skill("s1", "desc", "t", "code")
