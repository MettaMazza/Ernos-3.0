...
async def test_voice_manager_speak_edge_cases():
    ...
    with patch.object(manager.synthesizer, 'stream_audio', MagicMock(side_effect=mock_stream_audio)):
         # Patch the RawPCMSource
         with patch("src.voice.manager.RawPCMSource", MagicMock()):
             await manager.speak(123, "test")
