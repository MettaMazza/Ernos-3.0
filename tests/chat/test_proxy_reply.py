...
def mock_bot():
    ...
    mock_tape_engine.process = AsyncMock(return_value=("Proxy response from Ernos", [], []))
...
class TestProxyReplyHandler:
    ...
    async def test_full_proxy_flow(self, proxy_cog, mock_bot):
        ...
        proxy_cog.bot.cognition.process.assert_awaited()
        call_kwargs = proxy_cog.bot.cognition.process.call_args[1]
    ...
    async def test_proxy_cognition_failure(self, proxy_cog, mock_bot):
        ...
        proxy_cog.bot.cognition.process = AsyncMock(side_effect=RuntimeError("Engine exploded"))
        admin_dm = _make_dm_message(list(settings.ADMIN_IDS)[0], "")
        target = _make_target_message()
    ...
    async def test_proxy_empty_response(self, proxy_cog, mock_bot):
        ...
        proxy_cog.bot.cognition.process = AsyncMock(return_value=("", [], []))
        admin_dm = _make_dm_message(list(settings.ADMIN_IDS)[0], "")
        target = _make_target_message()
    ...
    async def test_proxy_long_response_chunks(self, proxy_cog, mock_bot):
        ...
        long_response = "a" * 2005
        proxy_cog.bot.cognition.process = AsyncMock(return_value=(long_response, [], []))
