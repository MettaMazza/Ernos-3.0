...
def mock_bot():
    ...
    mock_tape_engine.process = AsyncMock(return_value=("Response from bot", [], []))
    bot.tape_engine = mock_tape_engine
    bot.cognition = mock_tape_engine
...
def chat_listener(mock_bot):
    ...
    listener.preprocessor.process = AsyncMock(return_value={"complexity": "LOW", "intent": "chat"})
...
def mock_message():
    ...
    msg.channel.typing = MagicMock(return_value=mock_typing_context)
...
@pytest.mark.asyncio
async def test_on_message_ignore_self(chat_listener, mock_message):
    """Ernos ignores its own messages (self-loop prevention) but allows other bots."""
    mock_message.author.id = chat_listener.bot.user.id  # Ernos's own ID
