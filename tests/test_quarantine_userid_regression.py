...
class TestQuarantineInferUserId:
    ...
    def setup_method(self):
        ...
        bot_mock = MagicMock()
        bot_mock.hippocampus.graph.get_dominant_owner.return_value = None
        self.daemon = DreamConsolidationDaemon(bot=bot_mock)
...
class TestBuildUserLookup:
    ...
    def test_no_users_dir(self):
        with patch("src.daemons.dream_consolidation.data_dir") as mock_data_dir:
            from pathlib import Path
            mock_data_dir.return_value = Path("/nonexistent/path/for/test")
...
class TestProcessQuarantine:
    ...
    def setup_method(self):
        from src.daemons.dream_consolidation import DreamConsolidationDaemon
        self.bot = MagicMock()
        self.bot.hippocampus.graph.get_dominant_owner.return_value = None
