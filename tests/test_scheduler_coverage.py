...
class TestOneTimeTasks:
    """Tests for add_one_time_task method."""

    @pytest.mark.asyncio
    async def test_add_one_time_task(self):
        """Test adding and executing a one-time task."""
        ts = TaskScheduler()
        executed = []
        ...
        run_at = datetime.now() - timedelta(seconds=1)  # Set to past to trigger immediately
        ts.add_one_time_task("one_time", run_at, task)

        assert len(ts._adhoc_tasks) == 1
        await ts._check_and_run()
        assert len(executed) == 1
        assert len(ts._adhoc_tasks) == 0
