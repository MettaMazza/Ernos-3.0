...
def test_trinity_stack_assembly():
    ...
    with patch.object(PromptManager, '_read_file') as mock_read:
        ...
        assert "CONTENT:kernel_backup.txt" in prompt
        assert "CONTENT:identity_core.txt" in prompt
...
@pytest.mark.asyncio
async def test_preprocessor_malformed_json():
    """Verify PreProcessor gracefully handles non-JSON (tool calls) output."""
    mock_bot = MagicMock()
    mock_engine = MagicMock()
    # Simulate LLM returning a tool call instead of JSON
    mock_engine.generate_response.return_value = '[TOOL: read_file(path="message.txt")]'
    mock_bot.engine_manager.get_active_engine.return_value = mock_engine
    mock_bot.loop.run_in_executor = AsyncMock(side_effect=lambda e, f, *a: f(*a))
    ...
    # Assert fallback dictionary is returned
...
async def test_chat_integration_intent_injection():
    ...
    mock_bot.tape_engine = MagicMock()
    # run_tape_cycle returns (text, files, tool_outputs)
