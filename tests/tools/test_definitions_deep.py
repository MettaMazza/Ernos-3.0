...
async def test_start_deep_research_flow(mocker):
    ...
    with patch.dict("sys.modules", {"ddgs": mock_module}):
        ...
        assert "Preliminary data saved" in res
...
# --- Recall User ---
def test_recall_user_paths(mocker):
    # Mock Path sequence
    # Path("memory") / "public" ...
    ...
    # Chain of truedivs for path construction
    # Path("memory") / "public" / "users" / clean_id / "timeline.jsonl"
    # That's 4 divs: root / a / b / c / d -> final
    mock_root.__truediv__.return_value.__truediv__.return_value \
        .__truediv__.return_value.__truediv__.return_value = mock_final
        
    with patch("src.tools.recall_tools.Path", return_value=mock_root):
        from src.tools.memory import recall_user
        
        # 1. Not Found
        mock_final.exists.return_value = False
        res = recall_user("123")
        assert "No public silo" in res
        
        # 2. Empty
        mock_final.exists.return_value = True
        
        # We need to catch 'open' calls. 
        # Since recall_user is in src.tools.memory, it calls builtins.open
        # We'll use mock_open
...
def test_manage_project_actions(mocker):
    # Patch Path where manage_project is defined
    with patch("src.tools.coding.Path") as MockPath:
        # Import directly to ensure we test the function that uses this Path
        from src.tools.coding import manage_project
        
        mock_f = MagicMock()
        mock_f.exists.return_value = True
        mock_f.read_text.return_value = '{"key": "val"}'
        MockPath.return_value = mock_f
        
        # Get
        assert "val" in manage_project("get", "key")
        
        # Set
        manage_project("set", "key", "new")
        assert mock_f.write_text.called
        args = mock_f.write_text.call_args[0]
        assert "new" in args[0]
        
        # Init
        manage_project("init")
        assert mock_f.write_text.call_count >= 2
        args = mock_f.write_text.call_args[0]
        assert "created" in args[0]
        
        # List
        assert "key" in manage_project("list")
