...
class TestWebTools:
    ...
    def test_search_web_ok(self):
        ...
        ddgs_cls = MagicMock()
        ddgs_cls.return_value.__enter__ = MagicMock(return_value=inst)
        ddgs_cls.return_value.__exit__ = MagicMock(return_value=False)
        with patch("src.tools.web._get_ddgs", return_value=ddgs_cls):
            assert "R" in search_web("q")
    ...
    def test_search_web_empty(self, mock_bing, mock_google, mock_yahoo):
        inst = MagicMock()
        ...
        assert "No results found across DDGS" in res

    @patch("src.tools.web._fallback_yahoo_search", return_value=[])
    @patch("src.tools.web._fallback_google_search", return_value=[])
    @patch("src.tools.web._fallback_bing_search", return_value=[])
    def test_search_web_import(self, mock_bing, mock_google, mock_yahoo):
        with patch("src.tools.web._get_ddgs", return_value=None):
            res = search_web("q")
        mock_google.assert_called_once()
        mock_bing.assert_called_once()
        mock_yahoo.assert_called_once()
    ...
    @patch("src.tools.web._fallback_google_search", return_value=[])
    @patch("src.tools.web._fallback_bing_search", return_value=[])
    def test_search_web_err(self, mock_bing, mock_google, mock_yahoo):
        ddgs_cls = MagicMock()
        ddgs_cls.return_value.__enter__ = MagicMock(side_effect=RuntimeError("x"))
        ddgs_cls.return_value.__exit__ = MagicMock(return_value=False)
        with patch("src.tools.web._get_ddgs", return_value=ddgs_cls):
            res = search_web("q")
        mock_google.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_browser_concurrency_lock(self, monkeypatch):
        """Verify @serialize_browser_action prevents overlapping browser actions on the same session."""
        ...
        class MockPage:
            ...
            async def goto(self, url, **kwargs):
                out_order.append(f"start_{url}")
                import asyncio
                await asyncio.sleep(0.1)
                out_order.append(f"end_{url}")
        ...
        ms = MockSession("test_user")
        monkeypatch.setattr(_manager, "get_session", lambda uid: ms)
        monkeypatch.setattr("src.tools.browser._is_safe_url", lambda url: True)

        import asyncio
        await asyncio.gather(
            browser_open("http://1", user_id="test_user"),
            browser_open("http://2", user_id="test_user")
        )

        assert len(out_order) == 4
        assert out_order[0].startswith("start_") and out_order[1].startswith("end_")
        assert out_order[2].startswith("start_") and out_order[3].startswith("end_")
...
class TestDeepResearch:
    ...
    def _mock_globals(self, has_msg=False, has_channel=False, has_cerebrum=False):
        ...
        # Mock Engine for DeepResearch
        mock_engine = MagicMock()
        # Return a JSON list string so it satisfies both sub-query regex AND report writing string requirement
        mock_engine.generate_response.return_value = '["sub1"]'
        g.bot.engine_manager.get_active_engine.return_value = mock_engine

        async def mock_run_in_executor(executor, func, *args):
            return func(*args)
        g.bot.loop.run_in_executor = mock_run_in_executor
    ...
    def _deep_research(self, tmp_path, g, ddgs_mod, **kwargs):
        ...
        os.chdir(tmp_path)
        try:
            # Delete 'loop' so start_deep_research takes the synchronous fallback
            # (otherwise it fires-and-forgets with asyncio.create_task and the
            #  background research never completes before the test loop closes)
            if hasattr(g.bot, 'loop'):
                saved_loop = g.bot.loop
                del g.bot.loop
            else:
                saved_loop = None

            # Mock AgentSpawner so the research task completes
            mock_result = MagicMock()
            mock_result.status.value = "completed"
            mock_result.output = "Mock research report content"
            mock_result.error = None
            mock_spawner_mod = MagicMock()
            mock_spawner_mod.AgentSpawner.spawn = AsyncMock(return_value=mock_result)
            ...
            with patch.dict("sys.modules", {
                ...
                "src.agents.spawner": mock_spawner_mod,
                "discord": MagicMock(),
            }):
                result = self._run_sync(start_deep_research(**kwargs))
            # Restore loop for other tests
            if saved_loop is not None:
                g.bot.loop = saved_loop
            return result
        finally:
            os.chdir(old)
    ...
    def test_kg_extraction(self, tmp_path):
        ...
        # The new flow: web.py -> InteractionLobe -> ResearchAbility._extract_and_store_knowledge
        # which internally does: MemoryLobe -> OntologistAbility.execute() per triple
        ont = AsyncMock()
        ont.execute.return_value = "Learned connection: X -REL-> Y"

        memory_lobe = MagicMock()
        memory_lobe.get_ability.return_value = ont

        # Mock the LLM to return valid triples
        mock_engine = MagicMock()
        mock_engine.generate_response.return_value = '[{"subject": "AI", "predicate": "IS_A", "object": "Technology"}]'
        g.bot.engine_manager.get_active_engine.return_value = mock_engine

        # Mock researcher ability with a real _extract_and_store_knowledge
        from src.lobes.interaction.researcher import ResearchAbility
        researcher_mock = MagicMock(spec=ResearchAbility)
        # Use the real method bound to the mock
        researcher_mock._extract_and_store_knowledge = ResearchAbility._extract_and_store_knowledge.__get__(researcher_mock)

        interaction_lobe = MagicMock()
        interaction_lobe.get_ability.return_value = researcher_mock

        # cerebrum.get_lobe_by_name returns the right lobe per name
        def get_lobe(name):
            if name == "InteractionLobe":
                return interaction_lobe
            if name == "MemoryLobe":
                return memory_lobe
            return None
        g.bot.cerebrum.get_lobe_by_name.side_effect = get_lobe

        ddgs = self._mock_ddgs_mod()
        self._deep_research(tmp_path, g, ddgs, topic="Topic")
        # The ontologist should have been called (at least the metadata triple)
        ont.execute.assert_called()
    ...
    def test_user_dm_delivery(self, tmp_path):
        ...
        ddgs = self._mock_ddgs_mod([{"title": "R", "href": "http://x"}])
        self._deep_research(tmp_path, g, ddgs, topic="T", user_id="123", research_channel_id="99999")
