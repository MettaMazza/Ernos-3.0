...
class TestVerifyResponseIntegrity:
    ...
    def test_claim_without_tool_fails(self):
        """Test claim without tool execution fails."""
        valid, reason = self.ability.verify_response_integrity(
            "I checked the code and found the issue",
            []  # No tools executed!
        )
        ...
        assert "checked the code" in reason
    
    def test_multiple_violations(self):
        """Test multiple claim violations."""
        valid, reason = self.ability.verify_response_integrity(
            "I checked the code and verified in the database",
            []  # No tools for either claim
        )
        
        assert valid is False
        # Should have multiple violations
        assert "checked the code" in reason or "database" in reason
