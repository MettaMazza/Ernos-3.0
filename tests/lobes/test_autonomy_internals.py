...
def mock_autonomy():
    ...
    lobe.cerebrum.bot.cognition = MagicMock()
    lobe.cerebrum.bot.cognition.process = AsyncMock()
...
async def test_autonomy_one_shot(mock_autonomy):
    ...
    mock_autonomy.bot.cognition.process.return_value = ("A philosophical thought.", None, None)
    
    res = await mock_autonomy.execute("What is life?")
    
    assert "[DREAM]: A philosophical thought." in res
    mock_autonomy.bot.cognition.process.assert_awaited()
...
async def test_autonomy_loop_idle(mock_autonomy):
    ...
    async def sleep_side_effect(duration):
        if duration in (10, 120):  # Normal or Lite mode loop interval
    ...
    with patch("asyncio.sleep", side_effect=sleep_side_effect):
        ...
        responses = [
            ("Thinking... [TOOL: verify_thought(query='test')]", None, ["Verified."]),
            ("Done thinking.", None, [])
        ]
        mock_autonomy.bot.cognition.process.side_effect = responses
        
        # Ensure observe is awaitable (AsyncMock)
        ...
        # Lite mode: 1 step max, Normal: 2+ steps
        assert mock_autonomy.bot.hippocampus.observe.call_count >= 1
...
async def test_autonomy_tool_limit(mock_autonomy):
    ...
    responses = [
        ("1 [TOOL: add_reaction(emoji='1')]", None, ["Reacted 1"]),
        ("2 [TOOL: add_reaction(emoji='2')]", None, ["Reacted 2"]),
        ("3 [TOOL: add_reaction(emoji='3')]", None, ["Reacted 3"]),
        ("4 [TOOL: add_reaction(emoji='4')]", None, ["Reacted 4"]), # Should trigger limit
        ("5 Done", None, []) # No tool, breaks
    ]
    ...
    # Lite mode: 1 step max, Normal: up to 5
    assert mock_autonomy.bot.hippocampus.observe.call_count >= 1
