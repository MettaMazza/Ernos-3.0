...
def isolate_flux(tmp_path):
    """Give each test a clean flux.json in tmp_path and disable Autonomy Lite capping."""
    def _mock_get_path(self, user_id):
        user_dir = tmp_path / "users" / str(user_id)
        user_dir.mkdir(parents=True, exist_ok=True)
        return user_dir / "flux.json"
    with patch.object(FluxCapacitor, '_get_path', _mock_get_path):
