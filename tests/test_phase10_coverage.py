...
class TestPerceptionEngine:
    ...
    def test_ingest_buffer_cap(self):
        ...
        assert len(self.pe._input_buffer) == 200
...
class TestAuditAbility:
    ...
    def test_verify_integrity_violation(self):
        """Heuristic checks disabled — always passes now."""
...
class TestOllamaEngine:
    ...
    def test_generate_response_object_format_success(self):
        # Simulate the new ollama.GenerateResponse object format
        ...
        with patch.dict(sys.modules, {"ollama": MagicMock()}):
            ...
            e._model = "test"
            e._client = MagicMock()
            e._client.generate.return_value = MockGenerateResponse("hello object")
            result = e.generate_response("test prompt")
            assert result == "hello object"

    def test_generate_response_thinking_field(self):
        """Thinking models (e.g. Gemini Flash) populate `thinking` instead of `response`."""
        class MockThinkingResponse:
            def __init__(self):
                self.response = ""
                self.thinking = "This is the model's chain-of-thought output."
                self.eval_count = 200
                self.done_reason = "stop"
        ...
        with patch.dict(sys.modules, {"ollama": MagicMock()}):
            ...
            e._model = "test"
            e._client = MagicMock()
            e._client.generate.return_value = MockThinkingResponse()
            result = e.generate_response("test prompt")
            assert result == "This is the model's chain-of-thought output."
...
class TestVectorEnhancedOllamaEngine:
    def test_generate_response_success(self):
        with patch.dict(sys.modules, {"ollama": MagicMock(), "src.memory.vector": MagicMock()}):
            from src.engines.rag_ollama import VectorEnhancedOllamaEngine
            e = VectorEnhancedOllamaEngine.__new__(VectorEnhancedOllamaEngine)
            e._model = "test"
            e._client = MagicMock()
            e.embedder = MagicMock()
            e.vector_store = MagicMock()
            
            with patch.object(VectorEnhancedOllamaEngine, 'context_limit', new_callable=PropertyMock) as mock_cl, \
                 patch.object(VectorEnhancedOllamaEngine, '_num_predict', new_callable=PropertyMock) as mock_np:
                mock_cl.return_value = 50000
                mock_np.return_value = 1000
                
                # Dictionary format
                e._client.generate.return_value = {"response": "rag result"}
                result = e.generate_response("test prompt")
    ...
    def test_generate_response_object_format_success(self):
        ...
        with patch.dict(sys.modules, {"ollama": MagicMock(), "src.memory.vector": MagicMock()}):
            ...
            with patch.object(VectorEnhancedOllamaEngine, 'context_limit', new_callable=PropertyMock) as mock_cl, \
                ...
                # Object format
                e._client.generate.return_value = MockGenerateResponse("rag object result")
                result = e.generate_response("test prompt")
                assert result == "rag object result"

    def test_generate_response_thinking_field(self):
        """Thinking models populate `thinking` instead of `response`."""
        class MockThinkingResponse:
            def __init__(self):
                self.response = ""
                self.thinking = "RAG thinking chain output."
                self.eval_count = 300
                self.done_reason = "stop"
        ...
        with patch.dict(sys.modules, {"ollama": MagicMock(), "src.memory.vector": MagicMock()}):
            ...
            with patch.object(VectorEnhancedOllamaEngine, 'context_limit', new_callable=PropertyMock) as mock_cl, \
                ...
                e._client.generate.return_value = MockThinkingResponse()
                result = e.generate_response("test prompt")
                assert result == "RAG thinking chain output."
...
# SkillRegistry tests consolidated into tests/test_skills.py
