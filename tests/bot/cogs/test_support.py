...
def mock_bot():
    ...
    bot.user = MagicMock()
    bot.user.id = 12345
    # Explicitly set bot.cognition since and bot.tape_engine both might be checked
...
async def test_ignore_other_channels(support_cog):
    ...
    assert not isinstance(support_cog.bot.tape_engine, MagicMock) or not support_cog.bot.cognition.process.called
