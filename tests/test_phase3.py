...
class TestBrowserOpen:
    """Tests for the current browser_open session-based tool (replacing deleted browse_interactive)."""

    @pytest.mark.asyncio
    async def test_blocked_url(self):
        """Internal/metadata URLs should be blocked."""
        from src.tools.browser import browser_open
        result = await browser_open("http://169.254.169.254/metadata")
    ...
    async def test_session_created(self):
        """browser_open should create a session and navigate to URL."""
        mock_page = AsyncMock()
        ...
        mock_session.last_active = 0

        with patch("src.tools.browser._manager") as mock_mgr:
            mock_mgr.get_session = MagicMock(return_value=mock_session)
            from src.tools.browser import browser_open
            result = await browser_open("https://example.com", user_id="test_user")

        assert "Example Domain" in result or "example.com" in result

    @pytest.mark.asyncio
    async def test_exception_handling(self):
        """browser_open should handle errors gracefully."""
        mock_session = AsyncMock()
        mock_session.ensure_open = AsyncMock(side_effect=Exception("Launch failed"))
...
class TestContextStream:
    ...
    def _make_stream(self, tmp_path):
        ...
        with patch.object(ContextStream, "PERSIST_DIR", tmp_path):
            ...
            stream.bot = None
            stream.turns = []
            stream._states = {}  # Must init before state property
            from src.memory.stream import StateVector
    ...
    def test_get_context_with_state(self, tmp_path):
        stream = self._make_stream(tmp_path)
        # State is now per-scope — set a PUBLIC state and query as PUBLIC user
        from src.memory.stream import StateVector
        pub_state = stream._get_or_create_state("PUBLIC", "1")
        pub_state.summary = "Alice is asking about cats"
    ...
    def test_get_context_private_visible_to_owner(self, tmp_path):
        ...
        # PRIVATE turns should be HIDDEN from PUBLIC scope (privacy fix)
        ctx = stream.get_context(target_scope="PUBLIC", user_id="1")
        assert "my secret" not in ctx
        # But PRIVATE turns ARE visible in PRIVATE scope to the owner
        ctx_priv = stream.get_context(target_scope="PRIVATE", user_id="1")
        assert "my secret" in ctx_priv
    ...
    def test_save_and_load_state(self, tmp_path):
        ...
        # Set a scoped state
        pub_state = stream._get_or_create_state("PUBLIC", "1")
        pub_state.summary = "Testing state"
        pub_state.topics = ["testing"]
        stream._save_state()

        stream2 = self._make_stream(tmp_path)
        stream2._load()
        loaded_state = stream2._states.get("PUBLIC:1")
        assert loaded_state is not None
        assert loaded_state.summary == "Testing state"
        assert "testing" in loaded_state.topics
    ...
    async def test_update_state_vector_with_bot(self, tmp_path):
        ...
        turn = Turn("1", "Alice", "msg", "resp", time.time(), {}, scope="PUBLIC")
        await stream._update_state_vector(turn)
        # State is now per-scope: check the PUBLIC:1 state vector
        scoped_state = stream._states.get("PUBLIC:1")
        assert scoped_state is not None
        assert scoped_state.summary == "Updated summary"
        assert "Alice" in scoped_state.active_participants
