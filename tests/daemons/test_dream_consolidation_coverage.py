...
class TestCompressEpisodicMemories:
    ...
    async def test_no_users_dir(self, daemon, tmp_path):
        with patch("src.daemons.dream_consolidation.data_dir", return_value=tmp_path / "nonexistent"):
    ...
    async def test_skips_files(self, daemon, tmp_path):
        ...
        with patch("src.daemons.dream_consolidation.data_dir", return_value=tmp_path):
            result = await daemon._compress_episodic_memories()
        assert result == 0
...
class TestPruneKGNodes:
    ...
    async def test_prune_success(self, daemon):
        daemon.bot.hippocampus.graph.prune_orphan_nodes.return_value = {"pruned": 7}
        assert await daemon._prune_kg_nodes() == 7

    @pytest.mark.asyncio
    async def test_prune_empty_result(self, daemon):
        daemon.bot.hippocampus.graph.prune_orphan_nodes.return_value = {}
        assert await daemon._prune_kg_nodes() == 0

    @pytest.mark.asyncio
    async def test_prune_exception(self, daemon):
        daemon.bot.hippocampus.graph.prune_orphan_nodes.side_effect = Exception("neo4j down")
        assert await daemon._prune_kg_nodes() == 0
...
class TestPersistSentinelCache:
    ...
    def test_exception(self, daemon, tmp_path):
        sentinel = MagicMock()
        ...
        broken = MagicMock()
        broken.__truediv__ = MagicMock(side_effect=Exception("disk"))
        with patch("src.daemons.dream_consolidation.data_dir", return_value=broken):
            assert daemon._persist_sentinel_cache() is False
