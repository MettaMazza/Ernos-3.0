...
class TestWelcomeCog:
    ...
    async def test_on_member_join_no_cognition_init(self):
        ...
        with patch("src.engines.cognition.CognitionEngine") as MockCE:
            ...
            mock_cog.process = AsyncMock(return_value=("Hi!", []))
            with patch("src.prompts.manager.PromptManager") as MockPM:
...
class TestAgencyDaemon:
    ...
    @pytest.mark.asyncio
    async def test_tick_sleep(self):
        d, bot = self._make_daemon()
    ...
    @pytest.mark.asyncio
    async def test_tick_with_decision(self):
        d, bot = self._make_daemon()
        ...
        with patch("src.tools.weekly_quota.is_quota_met", return_value=True):
            with patch.object(d, "_consult_autonomy_lobe", new_callable=AsyncMock, return_value=decision):
                with patch.object(d, "_execute_decision", new_callable=AsyncMock) as mock_exec:
                    await d._tick()
                    mock_exec.assert_called_once_with(decision)
